...
word_frequency_decade3 <- freq.df.3[1:5,]
word_frequency_decade3 <- cbind(decade,word_frequency_decade3)
write.csv(word_frequency_decade3,"Decade3_Top25WordFrequency.csv", row.names = FALSE)

overall_sentiment_decade3 <- bing_dtm_3_sentiment %>% 
    count(document, sentiment, wt = count) %>%
    spread(sentiment, n, fill = 0) %>%
    mutate(sentiment= positive - negative)

overall_sentiment_decade3 <- overall_sentiment_decade3[, -c(1,4)]

overall_sentiment_decade3 <- cbind(decade,overall_sentiment_decade3)
overall_sentiment_decade3 <- as.data.frame(overall_sentiment_decade3)
write.csv(overall_sentiment_decade3,"Decade3_OverallSentiment.csv", row.names = FALSE)

nrc_count_decade3 <- nrc_count
nrc_count_decade3 <- cbind(decade,nrc_count_decade3)
write.csv(nrc_count_decade3,"Decade3_OverallbyEmotion.csv", row.names = FALSE)
